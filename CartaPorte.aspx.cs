    ...
    public static List<string> valida(string leg)
    {
        ...
        if (leg.Length > 0 && leg != "null" && leg != "")
        {
            ...
            catch (Exception)
            {
                result.Clear();
                result.Add("Error");
                result.Add("Segmento invalido");
            }
        }
        else { result.Add("Error3");}
        return result;
    }
